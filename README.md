...
| **contract** | каждая реализация порта против общего набора: in-memory и SQLAlchemy на настоящем Postgres |
| migrations | upgrade → downgrade → upgrade в CI: миграция обязана откатываться |
...
- [x] **M1** — Postgres, Alembic, SQLAlchemy-репозитории под контрактные тесты
